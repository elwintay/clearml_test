...
f = open("/gtt/data/wikievents/muc_format/template_dicts.json")
...
def convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        cutoff_length = max_seq_length_src - special_tokens_count
        tokens_list = [tokens[x:x+cutoff_length] for x in range(0, len(tokens),cutoff_length)]
        input_ids_list = []
        input_mask_list = []
        segment_ids_list = []
        position_ids_list = []
        label_ids_list = []
        for count,tokens in enumerate(tokens_list):
            ...
            for idx, token in enumerate(tokens):
                ...
                if len(src_tokens)==cutoff_length:
                ...
                for role in template:
                    if role == "incident_type":
                        ...
                        for span in template[role]:
                            if num_entity_span < max_num_entity_tgt and span[0] in range(count*max_seq_length_src,count*max_seq_length_src+src_token_length) and span[1] in range(count*max_seq_length_src,count*max_seq_length_src+src_token_length):
                                num_entity_span += 1
                                tgt_tokens.append(tokens[span[0]-count*max_seq_length_src]) # span start token
                                tgt_position_ids.append(token_offset_to_src_token_offset[span[0]-count*max_seq_length_src])
                                tgt_tokens.append(tokens[span[1]-count*max_seq_length_src]) # span end token
                                tgt_position_ids.append(token_offset_to_src_token_offset[span[1]-count*max_seq_length_src])
                    tgt_tokens.append(sep_token)
                    tgt_position_ids.append(len(src_tokens) - 1) # to confirm
